...
This is a [LLVM](https://llvm.org/) and [Clang](https://clang.llvm.org/) compiler toolchain built for kernel development.

This toolchain targets the AArch32, AArch64 and X86 architectures and it is not suitable for anything other than bare-metal development;
...
The build scripts (powered by [tc-build](https://github.com/ClangBuiltLinux/tc-build)) behind this can be found [here](https://github.com/GhostMaster69-dev/tc-build).

## Host compatibility

This toolchain is built on Ubuntu 22.04.2 LTS (jammy), which uses glibc 2.35, So compatibility with older distributions cannot be guaranteed. Other libc implementations (such as musl) are not supported.
...
Cosmic Clang has been designed to be easy-to-use compared to other toolchains, such as [AOSP Clang](https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/). The differences are as follows:
